...
## 🎬 Animated icons — built, working, parked (2026-08-07)

Owner: *"the animations are nice, store how they're done so I can do something
with that later."* Nothing in the app draws one today — the checkpoint row went
back to flat marks the same afternoon — but the whole path works end to end and
is committed, so picking it up later is choosing where, not building anything.
...
| `platform/public/reader/icons/*.json` | three icons: two doodle faces and a feedback mark |
...
⚠️ **Licence unsettled.** Free-tier icons may owe attribution; nothing here
records which plan applies. Settle before one ships to students.
