# WhatsApp Groups — Booklesss Marketing Tracker

Track every group, every post, and every conversion here.
Update after each drop. This is the live record.

---

## How to use this file

1. Add each group before you post to it
2. After posting: log the date, which lead magnet version, and the bit.ly link used
3. After 48h: log click count from bit.ly
4. Log any Slack joins you can attribute to that group

---

## Tracking link format

Each group gets a unique bit.ly link pointing to the Slack invite.
Format: `https://bit.ly/booklesss-[group-slug]`

Create each link at bit.ly before posting. Name them clearly so the dashboard is readable.

---

## Groups

### ZCAS University — Treasury Management

| # | Group Name | Members (est.) | Link Slug | First Drop | Clicks | Joins | Notes |
|---|-----------|---------------|-----------|-----------|--------|-------|-------|
| 1 | [add group] | — | `tm-group-1` | — | — | — | |
| 2 | [add group] | — | `tm-group-2` | — | — | — | |

### ZCAS University — General Finance / Accounting

| # | Group Name | Members (est.) | Link Slug | First Drop | Clicks | Joins | Notes |
|---|-----------|---------------|-----------|-----------|--------|-------|-------|
| 1 | [add group] | — | `fin-group-1` | — | — | — | |

### ZCAS University — Year / Class Groups

| # | Group Name | Members (est.) | Link Slug | First Drop | Clicks | Joins | Notes |
|---|-----------|---------------|-----------|-----------|--------|-------|-------|
| 1 | [add group] | — | `class-1` | — | — | — | |

### Other Student Communities

| # | Group Name | Members (est.) | Link Slug | First Drop | Clicks | Joins | Notes |
|---|-----------|---------------|-----------|-----------|--------|-------|-------|
| 1 | [add group] | — | `other-1` | — | — | — | |

---

## Lead Magnets in Use

| Version | File | Target Course | Status |
|---------|------|--------------|--------|
| v1 | `tm-lead-magnet_v1.pdf` | BBF4302 Treasury Management | Draft |

---

## Post Templates

Framework applied: Give First → One CTA → everything else inside the PDF.
Score each message before sending: Give First / Micro Commitment / Social Proof / Authority / Rapport / Scarcity / Shared Identity. Aim 5+/7.

---

### Template A — Friend group / casual (score: 6/7)

```
Part 1 of a free Treasury Management series — 3 concepts that show up in almost every exam.

[PDF attached]

Part 2 drops tomorrow. Join to get it directly: https://join.slack.com/t/bookless10/shared_invite/zt-3t42wx6yq-8OFwcZTqTbPpC2Dg0q__Cg
```

**Why it works:** Short. One ask. Series angle pulls people to join rather than wait.

---

### Template B — Official class group / larger group (score: 6/7)

```
Free Treasury Management study guide — 3 exam concepts, based on past paper patterns.

[PDF attached]

More dropping this week. Join the study group to get them: https://join.slack.com/t/bookless10/shared_invite/zt-3t42wx6yq-8OFwcZTqTbPpC2Dg0q__Cg
```

**Why it works:** Credible framing. Low ask. No pitch.

---

### Template C — After social proof exists (use once you have members — score: 7/7)

```
[X] Treasury Management students are using this to prep. Here's the free guide I started with.

[PDF attached]

Full series inside: https://join.slack.com/t/bookless10/shared_invite/zt-3t42wx6yq-8OFwcZTqTbPpC2Dg0q__Cg
```

**Why it works:** Social proof leads. Tight. One ask.

---

### Template D — Urgency drop (use April 14–17 only)

```
Free Treasury Management guide — dropping this before the founding rate closes on the 18th.

[PDF attached]

That offer is inside. Won't come back after the 18th.
```

**Why it works:** Real scarcity. Still leads with the free PDF, not the pitch.

---

### Scoring reminder (from cold outreach framework)
| # | Principle | Quick check |
|---|-----------|------------|
| 1 | Give First | Does the message lead with free value, not a pitch? |
| 2 | Micro Commitment | Is there only ONE ask? |
| 3 | Social Proof | Any numbers, names, or "X students"? |
| 4 | Authority | Does it sound confident? No "might help" hedging? |
| 5 | Rapport | Would a classmate send this? Passes the text message test? |
| 6 | Scarcity | Is there a real deadline or constraint mentioned? |
| 7 | Shared Identity | ZCAS / BBF4302 / same course = in-group signal? |

Adjust the message based on the group. More casual in friend groups, tighter in official class groups.

---

## Conversion Log

| Date | Group | Action | Result |
|------|-------|--------|--------|
| — | — | First drop | — |
