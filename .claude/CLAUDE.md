...
- The rest of the **checkpoint row** — the note button and its five verdicts — is **Tabler (free)** (owner's pick, 2026-08-09): `<TablerIcon name="mood-happy" muted />` draws the **outline**, and without `muted` it draws that name's **`-filled` twin**. Generated like the rest (`ICONS` in `scripts/gen-tabler-icons.mjs` → `npm run gen:tabler`), where each entry is a *base* name and the generator **throws if the `-filled` twin is missing** — not every Tabler outline has one, and a mark without it would ship with no selected state. **MIT**, so this is the one family in the app that owes no attribution.

  **Both states inherit `currentColor`, and that is the reason to prefer it over the two families it replaced.** The colour of the chosen state is now a CSS value in `.grasp-btn[data-active]` rather than a property of the artwork: grey outline at rest, `--color-ink` when chosen. Giving it a hue again is one line in `globals.css`.
  ...
  **The flag is a real flag again, and that is what the swap bought.** Freehand Free has no plain flag — its only match, `learning-programming-flag`, is a pennant with `</>` on it that reads as a code icon at 20px — so the note button had settled for `notes-paper`. Tabler draws `flag`/`flag-filled`, plus the plain lightbulb and warning triangle Ultimate lacked, so **no row in this menu is a substitute for a picture the set could not supply** — the first time that has been true.

  **The three answers carry their words again** (owner, 2026-08-09), in **Satoshi at 11px** under each mark (`.grasp-caption`), **one word each** — Hate / Save / Love. The words came off on 2026-08-02 and the five family swaps in the week since were all attempts to find a drawing that says "save for later" on its own; none does. The button is a column now — `.grasp-mark` holds the 28px glyph box and the hover plate, `.grasp-btn` is as wide as its caption — so the checkpoint row is `items-start`, aligning the marks rather than the boxes. The note button keeps no caption (its label is a question before it is answered and a verdict after) and keeps its `aria-label`; the three answers dropped theirs, taking their accessible name from the visible text so the two can't drift.

  **The 22px cluster gap survives only because the captions are one word.** It was dropped to 12 for a few hours while they read "Hate it" / "Save for later" / "Love it" — those buttons were 33–67px wide, so 22 between *buttons* was ~80px between *glyphs*, which is what the reader actually lines up. One word each puts every caption inside the 28px the mark box already claims, so the button is a square again and 22 means what it meant. **Any caption long enough to widen a button makes this number wrong**, and it has to be recomputed from the glyph spacing rather than kept.
