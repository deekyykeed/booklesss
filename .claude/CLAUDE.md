...
- The reader's **checkpoint row** — the note button, its five verdicts and the three answers — is **Streamline Freehand**, in **two sets at once** (owner's pick, 2026-08-09): `<FreehandIcon name="smiley-thrilled" muted />` draws **Freehand Line (Free)**, which inherits `currentColor` and is greyed by `.grasp-btn`; without `muted` it draws **Freehand Duotone (Free)**, which brings its own `#020202` line and `#0c6fff` accent. Generated like the rest (`ICONS` in `scripts/gen-freehand-icons.mjs` → `npm run gen:freehand`), and the generator **throws unless a name exists in both packages** — only 982 of their 1,000 names overlap, so a mark present in one would otherwise ship with no second state. CC BY 4.0.

  **This is a different mechanism from the rest/full pair in `ultimate.tsx`, and the distinction is the reusable part.** There, one multicolour body was rewritten by regex into a rest state, because that family has no line-only twin. Here the foundry ships two genuinely different drawings of the same subject, so nothing is rewritten — which is why the grey lives in CSS (one value, `.grasp-btn`'s `color`) instead of being baked into the generated file. **Prefer a real two-set pair over a derived one** whenever a family offers it.

  **`ultimate.tsx` and its generator were DELETED in the same change** — the checkpoint row was the only thing importing them, and this codebase's rule is that a reverted option lives in git rather than parked in the tree. The hand-inlined Ultimate copy in `reader/file-icons.tsx` is untouched and independent.

  **One thing the free set cost us, so it isn't rediscovered: there is no plain flag in it.** Its only match, `learning-programming-flag`, is a pennant with `</>` on it, which reads as a code icon at 20px. The note button draws `notes-paper` instead — which names the control more accurately anyway (`SectionNote`, `lib/step-notes`). Freehand *gains* two marks Ultimate lacked: a plain lightbulb and a plain warning triangle, so the "Clear" and "Something looks wrong" rows no longer stand in a tick and a cross.
