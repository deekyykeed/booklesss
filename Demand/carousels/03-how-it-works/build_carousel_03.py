...
def build():
    ...
    slide(c, grain, logo, 2, "FIRST",
          big=["One topic,", "one channel."],
          body=["Your course, split so you", "can keep up. No noise."])

    # 3 / 6 — Short steps
    slide(c, grain, logo, 3, "THEN",
          big=["Short PDFs."],
          body=["Each topic lands as a step", "you can read in minutes."])
    ...
    slide(c, grain, logo, 5, "EVERY WEEK",
          big=["New steps,", "every week."],
          body=["Miss one?", "It's still there."])
