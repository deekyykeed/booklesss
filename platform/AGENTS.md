...
auth (`(auth)/`, `auth/callback`), `/api/step-feedback`, `/api/outcome-ticks`,
static step pages in `public/steps/` (generated by `_dev/step-generator/` at
the repo root), and the `/steps` index. Students arrive via step links posted
in Slack.

**Steps are gated (re-enabled 2026-07-15).** `proxy.ts` requires a Supabase
session for `/steps` and every `/steps/*` page (including the static HTML) and
redirects signed-out visitors to `/login?next=<step>`. The gate is **fail-soft**:
until `NEXT_PUBLIC_SUPABASE_URL` + `NEXT_PUBLIC_SUPABASE_ANON_KEY` are set it is
a no-op, so an unconfigured deploy still serves steps rather than 500ing. This
reverses the 2026-07-12 "public links + Slack is the paywall" decision.
