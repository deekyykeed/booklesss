...
export function SectionNote({ lessonId, sectionId }: { lessonId: string; sectionId: string }) {
  ...
  return (
    <div ref={wrap} className="relative">
      ...
      {open && (
        /* Opens upward and to the right edge: the button sits at the end of the
           row near the bottom of a section, so a menu dropping down would land
           on the next section's first line. */
        <div
          role="menu"
          className="squircle absolute bottom-[calc(100%+8px)] right-0 z-30 flex w-[230px] flex-col gap-0.5 rounded-2xl border border-[#e0e0e0] bg-white p-1.5 shadow-[0_2px_4px_-1px_rgba(0,0,0,0.10),0_12px_24px_-8px_rgba(0,0,0,0.18)]"
        >
          {NOTES.map((n) => (
            <button
              key={n.id}
