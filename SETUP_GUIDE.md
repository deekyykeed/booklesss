...
- **Google Gemini 2.5 Flash** for AI content generation (~$0.42 per course)
...
Currently using **Gemini 2.5 Flash** for cost-effectiveness.

To use a different model, edit `lib/gemini.ts`:

```typescript
// Gemini 2.5 Pro (higher quality, ~$5/course)
export const model = genAI.getGenerativeModel({ model: 'gemini-2.5-pro' });

// Gemini 3 Pro (best quality, ~$7/course)
export const model = genAI.getGenerativeModel({ model: 'gemini-3-pro' });
```
...
*Free tier: 1,500 requests/day (~7-10 courses/day free)*
